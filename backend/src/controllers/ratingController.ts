...
export const createOrUpdateRating = [
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (error.code === 11000) { // Duplicate key error (should be caught by findOne above, but as a fallback)
          return res.status(409).json({ message: 'You have already rated this dish for this menu date.' });
      }
      res.status(500).json({ message: 'Server error processing rating.', error: error.message });
    }
  },
];

export const getMyRatingForDishOnMenu = [
    param('dishId').isMongoId().withMessage('Valid Dish ID is required.'),
    param('menuDateString').isISO8601().withMessage('Valid Menu Date (YYYY-MM-DD) is required.'),
    async (req: Request, res: Response) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        ...
        try {
            const menu = await Menu.findOne({ date: parsedMenuDate, 'items.dish': dishId });
            if (!menu) {
                return res.status(404).json({ message: 'Dish not found on any menu for this date.' });
            }
            ...
            res.status(500).json({ message: 'Server error fetching rating.', error: error.message });
        }
    }
];

export const deleteMyRating = [
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (rating.student.toString() !== student.id) {
        return res.status(403).json({ message: 'Forbidden. You can only delete your own ratings.' });
      }
      ...
      res.status(500).json({ message: 'Server error deleting rating.', error: error.message });
    }
  },
];

// For Admins or general display (e.g., showing all ratings for a dish)
export const getRatingsForDish = [
  param('dishId').isMongoId().withMessage('Valid Dish ID is required.'),
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const dish = await Dish.findById(dishId);
      if (!dish) { // No need to check isActive for admin view of ratings
      ...
      if (startDateString) {
          ...
          if (startDate) queryFilter.menuDate = { ...queryFilter.menuDate, $gte: startDate };
          else return res.status(400).json({ message: "Invalid start date."});
      }
      if (endDateString) {
          const endDate = parseDateToUTC(endDateString as string);
          if (endDate) queryFilter.menuDate = { ...queryFilter.menuDate, $lte: endDate };
          else return res.status(400).json({ message: "Invalid end date."});
      }
