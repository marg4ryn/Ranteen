...
const dropAllCollections = async () => {
  try {
    ...
    if (collectionNames.length === 0) {
      console.log(
        "ℹ️  Baza danych jest już pusta - brak kolekcji do usunięcia."
      );
      return;
    }
    ...
    const dropPromises = collectionNames.map(async (collectionName) => {
      try {
        ...
        console.log(
          `   ❌ Błąd przy usuwaniu kolekcji ${collectionName}: ${error.message}`
        );
    ...
    if (error.message.includes("MONGO_URI")) {
      console.error(
        "💡 Upewnij się, że zmienna MONGO_URI jest ustawiona w pliku .env"
      );
...
const confirmDrop = (): boolean => {
  ...
  if (nodeEnv === "development") {
    ...
    console.log(
      "💡 Użyj --force lub --yes aby pominąć to ostrzeżenie w przyszłości.\n"
    );
    return true;
  }
