...
const polishDishes = [
  // Dania główne
  {
    ...
    description: 'Tradycyjny kotlet schabowy panierowany w bułce tartej, podawany z gotowanymi ziemniakami',
    category: 'danie główne' as const,
    imageUrl: '/api/uploads/dish-image/kotlet-schabowy.jpg'
  },
  {
    name: 'Pierogi ruskie',
    description: 'Pierogi z nadzieniem z ziemniaków i twarogu, podawane z prażoną cebulką',
    category: 'danie główne' as const,
  ...
  {
    name: 'Gołąbki',
    description: 'Gołąbki z ryżem i mięsem w liściach kapusty, podawane z sosem pomidorowym',
    category: 'danie główne' as const,
    imageUrl: '/api/uploads/dish-image/golabki.jpg'
  },
  {
    name: 'Kurczak w sosie czosnkowym',
    description: 'Soczyste filety z kurczaka w kremowym sosie czosnkowym z ziołami',
    category: 'danie główne' as const,
    imageUrl: '/api/uploads/dish-image/kurczak-czosnkowy.jpg'
  },
  {
    name: 'Kotlety mielone',
    description: 'Domowe kotlety mielone z wołowiny i wieprzowiny z ziemniakami',
    category: 'danie główne' as const,
...
// Sample users data
const sampleUsers = [
  { name: 'Anna Kowalska', email: 'anna.kowalska@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Piotr Nowak', email: 'piotr.nowak@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Maria Wiśniewska', email: 'maria.wisniewska@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Jan Kowalczyk', email: 'jan.kowalczyk@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Katarzyna Zielińska', email: 'katarzyna.zielinska@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Michał Szymański', email: 'michal.szymanski@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Agnieszka Dąbrowska', email: 'agnieszka.dabrowska@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Tomasz Kozłowski', email: 'tomasz.kozlowski@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Magdalena Jankowska', email: 'magdalena.jankowska@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Łukasz Wójcik', email: 'lukasz.wojcik@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Ewa Krawczyk', email: 'ewa.krawczyk@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Paweł Kaczmarek', email: 'pawel.kaczmarek@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Justyna Piotrowska', email: 'justyna.piotrowska@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Rafał Grabowski', email: 'rafal.grabowski@student.edu.pl', role: 'student' as const, isApproved: true },
  { name: 'Monika Pawłowska', email: 'monika.pawlowska@student.edu.pl', role: 'student' as const, isApproved: true }
];
...
const selectDishesForMenu = (dishes: IDish[]): Types.ObjectId[] => {
  ...
  const otherDishes = shuffled.filter(d => 
    d.category !== 'danie główne' && d.category !== 'zupa' &&
...
const seedTimeRangeDatabase = async () => {
  try {
    ...
    console.log('Rozpoczynanie seedowania dla okresu 1 czerwca - 14 lipca 2025...');

    // 1. Create users
    console.log('Tworzenie użytkowników...');
    const existingUsers = await User.find({ role: 'student', isApproved: true });
    let allUsers = [...existingUsers];
    
    // Add new users if we don't have enough
    if (existingUsers.length < 10) {
      const newUsers = await User.insertMany(sampleUsers.slice(0, 15 - existingUsers.length));
    ...
    const newDishes = polishDishes.filter(dish => !existingDishNames.includes(dish.name));
    ...
    const existingMenuDates = (await Menu.find({}, 'date')).map(m => m.date.toISOString().split('T')[0]);
    ...
    for (const menu of allMenusInRange) {
      for (const dishId of menu.dishes) {
        ...
        const existingRatings = await Rating.find({ dish: dishId, date: menu.date });
        ...
        if (existingRatings.length === 0) {
          ...
          for (let i = 0; i < numberOfRatings && i < shuffledUsers.length; i++) {
    ...
    for (const menu of allMenusInRange) {
      for (const dishId of menu.dishes) {
        ...
        const existingComments = await Comment.find({ dish: dishId, date: menu.date });
        ...
        if (existingComments.length === 0) {
          ...
          if (Math.random() < 0.4) {
            ...
            for (let i = 0; i < numberOfComments && i < shuffledUsers.length; i++) {
