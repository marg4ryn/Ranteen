...
import { setup } from "./config/passport"; // Initializes passport strategies
...
app.use(
  cors({
    origin: process.env.CLIENT_URL || "http://localhost:5001", // Allow frontend to connect
...
if (!mongoURI || !sessionSecret) {
  console.error(
    "MONGO_URI or SESSION_SECRET not defined in .env file. Session store will not work."
  );
  process.exit(1);
}
